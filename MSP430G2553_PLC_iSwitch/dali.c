...
void TI_DALI_Init(unsigned int key){
 ...
 #if 0  //SJ4020812 - Not used in our project  
  /*** Setup P2.0 and P2.1 ****************************************************/
  P2DIR &= ~BIT0;       // Input P2.0
  P2SEL &= ~(BIT1+BIT0);
  P2IE &= ~(BIT1+BIT0);   //SJ3080611 - After negate P2IE will get 11111100
  P2DIR |= BIT1;        // Output high P2.1 (IDLE)
  P2OUT |= BIT1;        // HW configuration will short bus lines 
                        //(opto-relay on daughter board) when P2.1 is low
                        // this will be used for TX function.
 #endif
  ...
  // segment is empty.
 #if 0  //SJ5140912 - ORG. TI DALI.
  flash_ptr = (unsigned char *)INFO_B2;  //SJ1060611 - equiv 0x10A0
  tmp_ptr = (unsigned char *)INFO_D1;    //SJ1060611 - equiv 0x1000
  while((flash_ptr[MAX_LEVEL] == MASK))  //SJ1030912 - MAX_LEVEL is 3
  {
    flash_ptr = flash_ptr - DATA_SIZE;  //SJ5240812 - 0x10A0 - 32 = 0x1080, 0x1060, 0x1040, 0x1020, 0x1000
    if((flash_ptr < (unsigned char *)LOWEST_INFO_SPACE))  //SJ1060611 - equiv 0x1000
    {
      while(1);    //trap cpu, device has lost even the factory default settings
    }
  }
  //SJ5140912 - SJNOTE: If section info_b and info_c are empty, flash_ptr should be 0x1000 after this point
  // There is a special case when the data was written to D1 from B2
  // (segment C is erased)
  if((flash_ptr == (unsigned char *)INFO_B2) && (tmp_ptr[MAX_LEVEL] != MASK))  //SJ5140912 - true if both INFO_B2 and INFO_D1 are used.
  flash_ptr = tmp_ptr;
 #else  //SJ5140912 - Replaced. Aztech
  unsigned short crcvalue=0;
  unsigned short stored_crc=0;
  //const void *data

  flash_ptr = (unsigned char *)INFO_D1;  //SJ5140912 - equiv 0x1000
  infoC_ptr = (unsigned char *)INFO_C1;  //SJ3190912 - equiv 0x1040
  infoB_ptr = (unsigned char *)INFO_B1;  //SJ5141212 - equiv 0x1080
...
//#define CRC_CHECKSUM_LSB      9
  ...
  short_address = infoC_ptr[PLC_LONG_ADDR_1_LOC];  //SJ3190912 - Retrieve DALI short address from Info C.
    //SJ2111212 - iSwitch ----------
      ...
      gstate_var.Room_ID = (infoC_ptr[ROOM_ID_MSB_LOC] << 8) + infoC_ptr[ROOM_ID_LSB_LOC];
    #endif
  ...
  else
  {
    ...
    short_address = 0;//infoC_ptr[PLC_LONG_ADDR_1_LOC];       //SJ4200912 - Retrieve DALI short address from Info C.
    //SJ2111212 - iSwitch ----------
    #if 0//def I_SWITCH_PROJECT  //SJ1171212 - No longer needed as room ID is determined by DIP switch
  ...
  #ifdef SWITCH_CONTROLLER  //SJ5141212 - For device type switch only
  if (device_type >= 11)
  {
      for(i=0; i<SWITCH_OPER_SZ; i++)
      {
        gswitch_oper[i] = infoB_ptr[i];
      }
  }
  #endif  //SJ5141212 - End

#if 0  //SJ2180912 - ORG. Skip.
  /* Update the flash_ptr so that call to the TI_DALI_Flash_Update writes to  */
  /* the correct location in Information memory. */
  if(flash_ptr  == (unsigned char *)HIGHEST_INFO_SPACE)  //SJ1060611 - equiv - 0x10A0
  {
   flash_ptr = (unsigned char *)LOWEST_INFO_SPACE;  //SJ1060611 - equiv - 0x1000
  }
  else
  {
   flash_ptr = flash_ptr+DATA_SIZE;  //SJ2040912 - If comes here, it will be 0x1020
  }
...
unsigned char TI_DALI_Rx(unsigned char* rx_msg)
{
    ...
    //SJ1171212 - If device type is 6, need to check if message is indeed sent to the correct room ID.
    //if (device_type == 6)  //SJ4131212 - This is temporary solution. REMOVE WHEN NO LONGER NEEDED
    //{
    //}
...
//--- SJ1171212 - Start of flash write -----------------------------------------
    ...
    #ifdef SWITCH_CONTROLLER
    if (wrt_area == 1)  //SJ1171212 - Write to segment B 0x1080
    {
      infoB_ptr[0] = 0;  //SJ1171212 - Dummy write to info B
      FCTL1 = key+WRT;
      for (i=0; i<SWITCH_OPER_SZ; i++)  //SJ1171212 - SWITCH_OPER_SZ is 64
      {
        infoB_ptr[i] = gswitch_oper[i];
      }
    }
    else
    #endif
...
//  while (gstate_var.general_delay) ;
    ...
    //temp_infoC[CRC_CHECKSUM_MSB] = (crcvalue & 0xFF00) >> 8;
