...
//SJ2220113 - SJEXT_ADDR: Use below sttmt for DALI extended address
  #ifdef SJEXT_ADDR
    ggeneral_buffer[ndx++] = (short_address >> 8);  //SJ2220113 - G3 Long Address Byte 6; ndx=20
  #else  //SJ3200213 - ORG
...
//SJ1170912        ggeneral_buffer[ndx++] = 0x02;  //SJ3220812 - Tx PGA Attenuation ndx=13
    ...
    while (gstate_var.UART_Rx_duration)  //SJ4190712 - Wait for 8 seconds for UART_Rx to complete
    {
        if (gstate_var.UART_Rx_State == UART_PROCESSING)  //SJ1300712 - New method
        {
            ...
            //break;
#ifdef UART_ISSUE
//SJ4180713 - SJTO_TRY: Re-activate UART interrupt enable   //UART_ISSUE
UC0IE |= UCA0RXIE;
#endif
