...
//SJ2170712 - Maybe we should include UART_Rx_mode
    ...
    if (gstate_var.UART_Rx_State == UART_STANDBY)  //SJ4180713 - ORG
    //if ((gstate_var.UART_Rx_State == UART_STANDBY) && (receive_buf == MSG_TYPE__DATA_TRANSFER))  //SJ4180713 - Replaced
    {
    ...
    if (gstate_var.UART_Rx_State == UART_RECEIVING)  //SJ3281112 - Replaced
    {
      ...
      if (gstate_var.UART_Rx_ndx < DATA_BUFFER_SZ)
      {
        ...
        if (gstate_var.UART_Rx_ndx > 3)
        {
          ...
          #ifndef UART_ISSUE//1  //SJ4180713 -
            if (buf_ndx == gstate_var.Rx_payload_len)  //SJ2210812 - It's time to stop reading.
                gstate_var.UART_Rx_State = UART_PROCESSING;
          #else
//SJ4180713 - SJTO_TRY: Disable UART interrupt enable   //UART_ISSUE
//SJ4180713 - UC0IE &= ~UCA0RXIE;
            if (buf_ndx == gstate_var.Rx_payload_len)  //SJ2210812 - It's time to stop reading.
            {
                UC0IE &= ~UCA0RXIE;  //SJ4180713 - Stop receiving packet until current packet has been fully processed
                gstate_var.UART_Rx_State = UART_PROCESSING;
            }
          #endif
        }
...
//SJ1270812    unsigned char ndx=0;
  ...
  #if 0  //SJ3290513 - Not used for now. It will be used if device is installed with sensor.
...
//SJ3121212 - Not used in iSwitch project
    ...
    gstate_var.max_oper_hour++;
    //if (gstate_var.max_oper_hour > MAX_OPER_HOUR)  //SJ3290513 - Operation beyond maximum allowed hour. Raise flag to shutdown system
#endif
