...
#if 0  //SJ4200314 - Moved to JDSU_Gemini_Main.c
...
void i2c_usi_mst_send_n_ack(BOOL ack)
{
  ...
  if(ack)
  {
    USISRL = 0x00;  //SJ4200314 - Send ACK
  }
  else
  {
    USISRL = 0xFF;  //SJ4200314 - Send NACK
  }
  USICNT = (USICNT & 0xE0) + 1;
...
{
  if(USICTL1 & USISTTIFG)  //SJ4200314 - Start condition interrupt flag
  {
  ...
  if(USICTL1 & USIIFG)  //SJ4200314 - Counter interrupt flag.
  {
